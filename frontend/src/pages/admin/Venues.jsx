    ...
    <main className="admin-page">
      ...
      <div className="venues-list" style={{ marginTop: '24px' }}>
        {loading ? (
          ...
          venues.map((venue) => (
            <div key={venue.id} className="venue-card" style={{ opacity: actionLoading ? 0.7 : 1, marginBottom: '20px', backgroundColor: '#1e293b', padding: '24px', borderRadius: '12px', border: '1px solid #334155' }}>
              ...
              <div className="venue-card-info" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '16px', marginTop: '16px', borderTop: '1px solid #334155', paddingTop: '16px' }}>
                <div className="info-block">
                  ...
                  <span className="info-value" style={{ color: '#e2e8f0', display: 'block', fontWeight: 'bold', marginTop: '4px' }}>{venue.city}</span>
                </div>
                <div className="info-block">
                  <span className="info-label" style={{ color: '#64748b', fontSize: '12px' }}>Capacity</span>
                  <span className="info-value" style={{ color: '#e2e8f0', display: 'block', fontWeight: 'bold', marginTop: '4px' }}>{venue.capacity}</span>
                </div>
                <div className="info-block">
                  <span className="info-label" style={{ color: '#64748b', fontSize: '12px' }}>Contact Phone</span>
                  <span className="info-value contact-blue" style={{ display: 'block', fontWeight: 'bold', marginTop: '4px' }}>{venue.contact_phone || 'N/A'}</span>
                </div>
              </div>
              ...
              <div className="row-actions" style={{ marginTop: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #334155', paddingTop: '16px' }}>
                <div style={{ display: 'flex', gap: '8px' }}>
                  {(venue.status === 'pending' || !venue.status) && (
                    <>
                      <button className="table-action-btn success" onClick={() => handleStatusChange(venue.id, 'approved')} disabled={actionLoading} style={{ cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                        <CheckCircle size={13} /> Approve Venue
                      </button>
                      <button className="table-action-btn danger" onClick={() => handleStatusChange(venue.id, 'rejected')} disabled={actionLoading} style={{ cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                        <XCircle size={13} /> Reject Proposal
                      </button>
                    </>
                  )}
                  {venue.status === 'approved' && (
                    <span style={{ color: '#10b981', fontSize: '13px', fontWeight: 'bold', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                      <CheckCircle size={14} /> Legitimate Verified Location
                    </span>
                  )}
                  {venue.status === 'rejected' && (
                    <span style={{ color: '#ef4444', fontSize: '13px', fontWeight: 'bold', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
